...
# -*- coding: utf-8 -*-
"""
Оркестратор для управления запуском скриптов Wildberries.
Запускает price_updater.py (с продажами) каждый час,
и insert_wb.py (без продаж) каждые 15 минут с защитой от одновременного запуска.
"""
...
logger = logging.getLogger("wb_orchestrator")
...
class WBOrchestrator:
    ...
    def __init__(self):
        ...
        self.with_sales_interval = Config.WB_INTERVAL
        self.no_sales_interval = Config.WB_NO_SALES_INTERVAL

        # Флаги включения модулей
        self.with_sales_enabled = Config.WB_WITH_SALES_ENABLED
        self.no_sales_enabled = Config.WB_NO_SALES_ENABLED
        ...
        logger.info("=" * 80)
        logger.info("ИНИЦИАЛИЗАЦИЯ ОРКЕСТРАТОРА WILDBERRIES".center(80))
        logger.info("=" * 80)
        logger.info(f"📊 Модуль с продажами (price_updater): {'ВКЛЮЧЕН' if self.with_sales_enabled else 'ОТКЛЮЧЕН'}")
        logger.info(
            f"   Интервал запуска: {self.with_sales_interval // 3600}ч {(self.with_sales_interval % 3600) // 60}м")
        logger.info(f"📦 Модуль без продаж (insert_wb): {'ВКЛЮЧЕН' if self.no_sales_enabled else 'ОТКЛЮЧЕН'}")
        logger.info(f"   Интервал запуска: {self.no_sales_interval // 60} мин")
        logger.info("=" * 80)

    async def run_with_sales(self):
        """Запуск price_updater.py (с продажами)"""
        ...
        async with self.execution_lock:  # Блокируем выполнение других модулей
            ...
            try:
                logger.info("-" * 80)
                logger.info("ЗАПУСК МОДУЛЯ С ПРОДАЖАМИ (price_updater)".center(80))
                logger.info("-" * 80)
    ...
    async def run_no_sales(self):
        """Запуск insert_wb.py (без продаж) с защитой от одновременного запуска"""
        ...
        async with self.execution_lock:
            try:
                logger.info("-" * 80)
                logger.info("ЗАПУСК МОДУЛЯ БЕЗ ПРОДАЖ (insert_wb)".center(80))
                logger.info("-" * 80)

                from insert_wb import NoSalesPriceUpdater
    ...
    async def run(self):
        ...
        logger.info("=" * 80)
        logger.info("ЗАПУСК ОРКЕСТРАТОРА WILDBERRIES".center(80))
        logger.info("=" * 80)
